<template>
    <div class="cl-route-nav">
        ...
        <template v-else>
            <el-breadcrumb>
                <el-breadcrumb-item v-for="(item, index) in list" :key="index">{{ (item.meta && item.meta.label) || item.name }}</el-breadcrumb-item>
            </el-breadcrumb>
        </template>
    </div>
</template>
    ...
    setup() {
        ...
        watch(
            ...
            (val: any) => {
                ...
                list.value = _(store.getters.menuGroup).map(deep).filter(Boolean).flattenDeep().value();
