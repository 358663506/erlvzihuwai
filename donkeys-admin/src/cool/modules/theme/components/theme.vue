        ...
        <el-drawer v-model="drawer.visible" title="系统设置" size="300px">
            <div class="cl-theme__container">
                <div class="cl-theme__color is-card">
                    ...
                    <ul>
                        <el-tooltip v-for="(item, name) in themeList" :key="name" :content="item.label" placement="top">
                            <li
                                :style="{
                                ...
                                @click="setTheme(item)"
                            >
                                <i v-show="item.color == form.theme.color" class="el-icon-check"></i>
                            </li>
                        </el-tooltip>
                    </ul>
                </div>
                ...
                <div class="cl-theme__switch is-card">
                    ...
                    <ul>
                        <li v-if="!browser.isMini">
                            <span>显示一级菜单栏</span>
                            <el-switch v-model="form.conf.showAMenu" size="mini" @change="onAMenuChange" />
                        </li>
                        <li>
                            <span>显示路由导航栏</span>
                ...
                <div class="cl-theme__tips" v-if="isDev">
                    <el-alert type="warning" :closable="false" show-icon title="手动修改配置文件可设置为默认主题、布局。" />

                    <el-button round type="primary" size="small" style="width: 100%" :disabled="!form.theme.url" @click="openDesc">修改说明</el-button>
                </div>
            </div>
        </el-drawer>
