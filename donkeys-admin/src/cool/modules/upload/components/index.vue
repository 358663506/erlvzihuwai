                    ...
                    <template v-else>
                        <template v-if="_file">
                            ...
                            <div class="cl-upload__actions">
                                <span v-if="_file.type == 'image'" class="cl-upload__actions-preview" @click.stop="_onPreview(_file)">
                                    <i class="el-icon-zoom-in"></i>
                                </span>

                                <span class="cl-upload__actions-delete" @click.stop="_onRemove(_file)">
...
export default {
    ...
    methods: {
        ...
        async httpRequest(req) {
            const { mode, type } = await this.uploadMode();
            // 多种上传请求
            const upload = (file) => {
                return new Promise((resolve, reject) => {
                    const next = (res) => {
                        ...
                        if (type == 'qiniu') {
                            data.append('token', res.uploadData.token);
                            data.append('key', res.uploadData.key);
                        } else {
                    ...
                    if (mode == 'local') {
                        ...
                        this.service.base.common
                            .upload({ fileName: file.name.replace(/\s+/g, '_'), folder: this.fileDir })
                            .then((res) => {
                                next({ ...res, host: res.uploadUrl });
                            })
                            .catch(reject);
                    } else {
                        this.service.base.common
        ...
        // 上传模式
        uploadMode() {
            return this.service.base.common.uploadMode().then((res) => res);
        }
    }
};
</script>
